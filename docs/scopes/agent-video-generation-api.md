...
- Returning typed validation warnings or errors when mismatch exceeds a
  threshold.
- Allowing agents to choose whether to shorten narration, extend timeline, or
  render to the longer media duration.
...
- Fail or warn when selected audio and timeline durations mismatch beyond a
  configured threshold.
- Add `rewrite_script` and `extend_timeline` alignment job modes if feasible in
  v1; otherwise expose `fail` and `render_longest` first.
...
- Add per-workspace quotas for generated asset count, total media duration, and
  export minutes.
...
- Should prompt-only generation default to mostly images plus narration, or
  include generated video clips by default despite cost and latency?
- What is the default hosted quota for generated videos per project?
- Should composition planning automatically choose providers, or should agents
  provide allowed provider lists for every request?
- Should audio alignment happen before timeline generation, before export, or
  both?
- How much generated asset provenance can be exposed without leaking sensitive
